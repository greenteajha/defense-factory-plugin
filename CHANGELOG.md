...
- The `defense-factory-review` skill now runs stage 1 (threat model), then stage 2 (finding discovery), then stage 3 (isolated validation) in one run, and reports all three in one summary. If Docker becomes unavailable after the prerequisite check passed, it reports stages 1 and 2 and records stage 3 as `inconclusive` rather than discarding them. Earlier it ran only stages 1 and 2.
