...
An early, provider-neutral application security plugin inspired by [OpenAI's Defense Factory](https://openai.com/the-defense-factory/). Version 0.4.0 (in development; the current release is 0.3.0) implements stages 1 and 2 of a [six-stage workflow](docs/workflow-contracts.md), the `threat-model` and `finding-discovery` skills, plus `defense-factory-review`, which runs them in order, with portable manifests and client adapters for Claude Code, ChatGPT/Codex, and Cursor. It is an independent project, not an OpenAI product or an implementation of OpenAI's full Defense Factory.
...
- `skills/defense-factory-review` runs the implemented stages in order in one run when you ask for a Defense Factory review: stage 1, then stage 2 without further prompts, then one combined summary.
...
skills/defense-factory-review/      runs stages 1 and 2 in order: SKILL.md, evals
...
Installing this version adds the `threat-model`, `finding-discovery`, and `defense-factory-review` skills. Ask for "a Defense Factory review" of a repository to run stages 1 and 2 in one go, or ask for a threat model (stage 1) or security findings (stage 2) on their own, or invoke any skill by name. Skills for later stages will be added under `skills/` as they are implemented.
