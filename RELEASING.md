...
3. Run `python3 scripts/check-package.py`, `python3 -m unittest discover -s tests`, and `claude plugin validate .`. Review the file inventory for secrets and unnecessary assets. Confirm each skill's positive trigger, non-trigger, missing-access behavior, and evidence output.
