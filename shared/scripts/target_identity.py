#!/usr/bin/env python3
"""Report the stable identity and version of a Defense Factory target.
...
def resolve(start, scope_items):
    """Return (repository root, in_git, scope paths relative to the root). Raises ValueError."""
    start = Path(start).expanduser().resolve()
    if not start.is_dir():
        raise ValueError(f"{start} is not a readable directory")
    top = git(start, "rev-parse", "--show-toplevel")
    in_git = top is not None
    root = Path(top).resolve() if in_git else start

    scope = []
    for item in scope_items or [str(start)]:
        path = (start / item).resolve() if not Path(item).is_absolute() else Path(item).resolve()
        if path != root and root not in path.parents:
            raise ValueError(f"scope {item} is outside {root}")
        if not path.exists():
            raise ValueError(f"scope {item} does not exist")
        scope.append(path.relative_to(root).as_posix() or ".")
    return root, in_git, scope


def identify(start, scope_items=()):
    """The identity report printed by main(), as a dict. Raises ValueError on bad arguments."""
    root, in_git, scope = resolve(start, scope_items)
    remote = None
    if in_git:
...
def main():
    ...
    try:
        report = identify(args.root, args.scope)
