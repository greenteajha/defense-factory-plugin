...
2. Otherwise run `scripts/find_threat_model.py --root <target>` (add `--out-dir <folder>` if the user chose a location for stage 1). It checks every run copy and selects the newest one that passes `check_model.py`, has status `complete` or `inconclusive`, and still matches the code (its version recomputed for the model's own scope must be identical). Use its `threat_model` path and `run_id`. A model made on a moved or copied folder (for example in a cloud workspace) is accepted when its version matches exactly; the script reports this in `notes`, and `start_findings.py` records it as an assumption. A different repository (a different Git remote) is never accepted.
