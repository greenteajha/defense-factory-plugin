...
Default: <git top level>/.defense-factory/, or <root>/.defense-factory/ for a target that is not a
Git repository, containing a .gitignore with "*" so Git ignores the folder and everything in it,
then runs/<run-id>/<stage>/ inside it. Later stages find earlier runs there. Target identity
digests never include .defense-factory, so writing there does not change the target's version.
...
def main():
    ...
    if args.out_dir:
        ...
        base = (git_root or root) / ".defense-factory"
