...
1. **A container engine.** Run `scripts/check_environment.py` first. It must find a usable engine (Docker Desktop, Podman, Colima, or Rancher Desktop) with a running daemon and enough memory. If it does not, stop and tell the user exactly what to install or start; never run the target on the host.
...
5. **Export the target and build the workbench.** `scripts/export_target.py --root <target> --scope <path> --out <stage_dir>/container-build --run-id <run_id>`, then build a base image for the target's stack with the printed `--label` arguments. Record the base image and its digest.
6. **For each finding, in priority order** (see [references/method.md](references/method.md)):
   ...
   2. **Choose the strongest feasible method:** reproduction through the app's real interface, an added focused test, a crash/sanitizer/debugger trace, or the static fallback in [references/static-finding-assessment.md](references/static-finding-assessment.md) when dynamic execution is blocked or disproportionate.
