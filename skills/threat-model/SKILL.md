...
1. **Authorization gate.** Before reading any source, confirm that the user is authorized to assess the target and state the scope (whole repository, or specific paths). Accept an explicit statement in the request, or an authorization already recorded by the calling workflow. If authorization is absent or unclear, ask. If it is refused or still unclear, stop with status `blocked`. Record the confirmation and scope in the output header.
...
2. **Prepare output storage.** Run `scripts/prepare_workspace.py --root <target>` to create the self-ignoring `.defense-factory/` folder and this run's stage folder, and to get the run ID and current UTC timestamp for the header (never estimate the time), unless the user named another location (pass it with `--out-dir`). If the script reports that the location is not ignored by Git or the target is not a Git repository, ask the user where to save output before writing anything. Mention once that ignored files still travel in archives, container build contexts, and synced folders.
3. **Choose how the model is obtained.** Follow [references/inputs-and-reuse.md](references/inputs-and-reuse.md) to decide, in order: preserve a supplied model; let authoritative repository guidance stand in; reuse the stored repository model when its identity and version match; or generate a new model. That reference also defines when the stored reusable model may be read or replaced.
...
8. **Write the model.** Fill [assets/threat-model-template.md](assets/threat-model-template.md): record header, the four model sections, the hypotheses table with stable IDs (`TM-H1`, `TM-H2`, …), the canonical six-field summary, coverage gaps, and open questions. See [references/record-and-status.md](references/record-and-status.md) for header fields and status values.
9. **Verify before saving.** Run `scripts/check_citations.py --repo <repo root> <model file>` and correct or remove every citation it rejects. Then check that the model stays within scope, describes actual runtime boundaries, keeps hypotheses separate from findings, lists coverage gaps, and contains no secret values.
...
- has a filled record header with identity, version, scope, authorization, status, and next action;
- traces each important claim to a verified `path:line` citation or a labeled source (user context, knowledge base, policy);
- accounts for every material boundary with a hypothesis, an explained control, or an open question;
...
- `scripts/target_identity.py`, `scripts/prepare_workspace.py`, `scripts/resolve_security_md.py`, `scripts/check_citations.py`: deterministic helpers. Each prints JSON or Markdown to standard output, documents its exit codes in `--help`, and needs only Python 3.9 or later.
